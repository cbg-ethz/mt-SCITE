...
/* creates the content for the GraphViz file from a parent vector, using the gene names as node labels */
std::string getGraphVizFileContentNames2(int* parents, int n, vector<string> geneNames, bool attachSamples, bool** ancMatrix, int m, double** logAltLL, double** logRefLL){
    std::stringstream content;
    content << "digraph G {\n";
    content << "node [color=deeppink4, style=filled, fontcolor=white];\n";
    ...
    if(attachSamples==true){
        ...
        content << "node [color=lightgrey, style=filled, fontcolor=black];\n";
        std::string attachment = getBestAttachmentString2(ancMatrix, n, m, logAltLL, logRefLL, geneNames);
        content << attachment;
    }
    content <<  "}\n";
    return content.str();
...
/* creates the attachment string for the samples, the optimal attachment points are recomputed from scratch based on ref and alt allele log likelihoods */
std::string getBestAttachmentString2(bool ** ancMatrix, int n, int m, double** logAltLL, double** logRefLL, vector<string> geneNames){
    bool** matrix = attachmentPoints2(ancMatrix, n, m, logAltLL, logRefLL);
    std::stringstream a;
    for(int i=0; i<=n; i++){
        for(int j=0; j<m; j++){
            if(matrix[i][j]==true){
...
/* This is a re-computation of the best attachment points of the samples to a tree for printing the tree with attachment points */
/*   gets an ancestor matrix and returns a bit matrix indicating the best attachment points of each sample based on the error log scores */
bool** attachmentPoints2(bool ** ancMatrix, int n, int m, double** logAltLL, double** logRefLL){
    ...
    for(int sample=0; sample<m; sample++){       // foreach sample
        double bestAttachmentScore = 0.0;     // currently best score for attaching sample
        for(int gene=0; gene<n; gene++){   // start with attaching node to root (no genes mutated)
            bestAttachmentScore += logRefLL[sample][gene];
        }
        for(int parent=0; parent<n; parent++){      // try all attachment points (genes)
            double attachmentScore=0.0;
            for(int gene=0; gene<n; gene++){     // sum up scores for each gene, score for zero if gene is not an ancestor of parent, score for one else wise
                //remove//attachmentScore += logScores[dataMatrix[sample][gene]][ancMatrix[gene][parent]];
                //TODO: double check with Katharina
                attachmentScore += ancMatrix[gene][parent]*logAltLL[sample][gene] + (1-ancMatrix[gene][parent])*logRefLL[sample][gene];
            }
            if(attachmentScore > bestAttachmentScore){
                bestAttachmentScore = attachmentScore;
            }
