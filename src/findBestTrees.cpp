...
double** getProbDataMatrix(int n, int m, string fileName);
...
int main(int argc, char* argv[])
{
    ...
    std::vector<struct Tree> optimalTrees2;            // list of optimal trees found by MCMC
	std::string sampleOutput;                            // the samples taken in the MCMC as a string for outputting
    std::string sampleOutput2;                           // the samples taken in the MCMC2 (prob input version) as a string for outputting



	/**  read parameters and data file  **/
	readParameters(argc, argv);
    //remove//int** dataMatrix = getDataMatrix(n, m, fileName);
    double** dataMatrix2 = getProbDataMatrix(n, m, fileName);
    double** logAltLL = getlog_doubleMatrix(dataMatrix2, m, n);
    double** logRefLL = getlog_complement_doubleMatrix(dataMatrix2, m, n);

    print_doubleMatrix(dataMatrix2, 4, 5);
    cout << endl;
    print_doubleMatrix(logAltLL, 4, 5);
    cout << endl;
...
//	double* errorRates = getErrorRatesArray(fd, ad1, ad2, cc);
	...
	writeToFile(sampleOutput2, sampleOutputFile.str());
...
//		}
        ...
        // TODO: fix this part and add later
//        if(treeType == 'm'){
            string output;
            output = getGraphVizFileContentNames2(parentVector, parentVectorSize, getGeneNames(geneNameFile, n), attachSamples, ancMatrix, m, logAltLL, logRefLL);
...
//	delete [] logScores;
//	delete [] errorRates;
    //remove//free_intMatrix(dataMatrix);
    free_doubleMatrix(dataMatrix2);
    free_doubleMatrix(logAltLL);
    free_doubleMatrix(logRefLL);
    cout << optimalTrees2.size() << " opt trees \n";
    emptyVectorFast2(optimalTrees2, n);
...
double** getProbDataMatrix(int n, int m, string fileName){
    ...
    double** dataMatrix = init_doubleMatrix(n, m, -1);
    ...
    in.close();
    double** transposedMatrix = transposeDoubleMatrix(dataMatrix, n, m);
    free_doubleMatrix(dataMatrix);
