...
/* This runs the MCMC for learning the tree, it samples from the posterior and/or records the optimal trees */
std::string runMCMC2(vector<struct Tree>& bestTrees, int noOfReps, int noOfLoops, double gamma, vector<double> moveProbs, int n, int m, double** logAltLL, double** logRefLL, char scoreType, int* trueParentVec, int step, bool sample, bool useTreeList, char treeType){
    ...
    int parentVectorSize = n;
    if(treeType=='t'){parentVectorSize = (2*m)-2;}                     // transposed case: binary tree, m leafs and m-1 inner nodes, root has no parent
    //remove//double betaPriorMean = errorRates[1] + errorRates[2];             // AD1 + AD2 the prior mean for AD error rate
    //remove//double betaPriorSd   = priorSd;                                     //  prior sd for AD error rate
    //remove//double bpriora = ((1-betaPriorMean)*betaPriorMean*betaPriorMean/(betaPriorSd*betaPriorSd)) - betaPriorMean;     // <-10.13585344 turn the mean and sd into parameters of the beta distribution
    //remove//double bpriorb = bpriora*((1/betaPriorMean)-1);           //<-13.38666556
    //remove//double jumpSd = betaPriorSd/chi;                          // chi: scaling of the known error rate for the MH jump; resulting jump sd
    //cout << "betaPriorMean: " << betaPriorMean << "\n";
    //cout << "betaPriorSd:   " << betaPriorSd << "\n";
    //cout << "bpriora:       " << bpriora << "\n";
    //cout << "bpriorb:       " << bpriorb << "\n";
    ...
    double bestTreeLogScore = -DBL_MAX;          // log score of T in best (T,beta)
    double bestScore = -DBL_MAX;                 // log score of best combination (T, beta)
    //remove//double bestBeta = betaPriorMean;
    stringstream sampleOutput;
    ...
    for(int r=0; r<noOfReps; r++){   // repeat the MCMC, start over with random tree each time, only best score and list of best trees is kept between repetitions
        ...
        bool** currTreeAncMatrix =  parentVector2ancMatrix(currTreeParentVec,parentVectorSize);
        //remove//double** currLogScores = getLogScores(errorRates[0], errorRates[1], errorRates[2], errorRates[3]);           // compute logScores of conditional probabilities
        ...
        for(int it=0; it<noOfLoops; it++){                                     // run the iterations of the MCMC
            if(it % 100000 == 0){ cout << "At mcmc repetition " << r+1 << "/" << noOfReps << ", step " << it << ": best tree score " << bestTreeLogScore << " and best overall score " << bestScore << "\n";}
            ...
            /* If the list of optimal trees is used, update it */
            if(useTreeList){
                updateTreeList2(bestTrees, currTreeParentVec, parentVectorSize, currScore, bestScore);
            }

            /* Sample from the posterior if required and past the burn-in phase */
            if(sample && it>=burnIn && it % step == 0){
                sampleOutput << sampleFromPosterior2(currTreeLogScore, parentVectorSize, currTreeParentVec);
            }
        ...
        delete [] currTreeParentVec;
        //remove//free_doubleMatrix(currLogScores);
        free_boolMatrix(currTreeAncMatrix);
    }                                              // last repetition of MCMC done
...
/* prints out the current tree and beta to sample from the posterior distribution */
string sampleFromPosterior2(double currTreeLogScore, int n, int* currTreeParentVec){
