...
void updateTreeList2(vector<struct Tree>& bestTrees, int* currTreeParentVec, int n, double currScore, double bestScore){

    if(currScore > bestScore){
        //cout << "tree list of size " << bestTrees.size() << " emptied\n";
        resetTreeList2(bestTrees, currTreeParentVec, n);                              // empty the list of best trees and insert current tree

    }
    else if (currScore == bestScore){
        if(!isDuplicateTreeFast2(bestTrees, currTreeParentVec, n)){               // if the same tree was not previously found
            Tree newElem = createNewTreeListElement2(currTreeParentVec, n);
            bestTrees.push_back(newElem);        // add it to list
        }
    }
}
...
/* removes all elements from the vector and inserts the new best tree */
void resetTreeList2(vector<struct Tree>& bestTrees, int* newBestTree, int n){
    emptyVectorFast2(bestTrees, n);                                         // empty the list of best trees
    Tree newElem = createNewTreeListElement2(newBestTree, n);
    bestTrees.push_back(newElem);                // current tree is now the only best tree
}
...
/* returns true if the same treeBeta was found before */
